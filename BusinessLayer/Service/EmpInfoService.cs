...
namespace BusinessLayer.Service
{
  public class EmpInfoService : BaseController<emp_info>, IEmpInfoService
  {
    ...
    public async Task<Response<emp_info>> UpdateEmployee(emp_info emp)
    {
      var response = new Response<emp_info>();
      try
      {
        ...
        DynamicParameters param1 = new DynamicParameters()
            .AddParam("@id", emp.emp_id);
        var check = _provider.ExecuteReader("emp_id_get", param1, commandType: CommandType.StoredProcedure);
        if (((DbDataReader)check).HasRows == true)
        {
          await _provider.QueryFirstOrDefaultAsync<emp_info>("emp_info_update", param, commandType: CommandType.StoredProcedure);
          response.Data = emp;
          response.successResp();
        }
        else
